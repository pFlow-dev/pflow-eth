...
CREATE OR REPLACE FUNCTION get_eth_transactions(addresses TEXT[], block_number INT)
    RETURNS TABLE
            (
...
CREATE OR REPLACE FUNCTION get_eth_transactions(address TEXT, block_number INT)
    RETURNS TABLE
            (
                transaction_hash    TEXT,
                transaction_details JSONB,
...
BEGIN
    ...
    WHILE max_block_number < latest_block_number
        LOOP
...
BEGIN
    ...
    RETURN QUERY
        SELECT max(block_number)                       AS highest_index,
...
BEGIN
    ...
    RETURN trunc(val_a + val_b + val_c + val_d + val_e)
        FROM (SELECT concat('x00000000', substr(val, 1, 8))::bit(64)::int8::numeric(78, 0) *
                     2 ^ 224::numeric(78, 0)                                                                 AS val_a,
                     concat('x00', substr(val, 9, 14))::bit(64)::int8::numeric(78, 0) * 2 ^
                                                                                        168::numeric(78, 0)  AS val_b,
                     concat('x00', substr(val, 23, 14))::bit(64)::int8::numeric(78, 0) * 2 ^
                                                                                         112::numeric(78, 0) AS val_c,
                     concat('x00', substr(val, 37, 14))::bit(64)::int8::numeric(78, 0) * 2 ^
                                                                                         56::numeric(78, 0)  AS val_d,
...
BEGIN
    ...
    api_endpoint :=
            config('service') || '/v0/signal?action=' || action::text || '&scalar=' || scalar::text || '&nonce=' ||
            request_id::text;
...
BEGIN
    ...
    INTO response
    FROM http_post(
            api_endpoint || '/v0/signal?action=' || actions_str || '&scalar=' || scalars_str || '&nonce=' ||
            request_id::TEXT,
            '',
            'application/json'
         );
...
BEGIN
    ...
    WITH deleted AS (
        DELETE FROM sent_transactions
            WHERE transaction_hash IN (SELECT t.transaction_hash
                                       FROM sent_transactions st
                                                join transactions t on t.transaction_hash = st.transaction_hash)
            RETURNING *)
    SELECT COUNT(*)
    INTO matched_count
    FROM deleted;
