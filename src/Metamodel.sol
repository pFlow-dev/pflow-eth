...
abstract contract MetamodelUint8 is Uint8ModelFactory {
    ...
    function cell(int8 initial, int8 capacity, Uint8Model.Position memory position) internal returns (Uint8Model.Place memory) {
        Uint8Model.Place memory p =  Uint8Model.Place(uint8(places.length), position, initial, capacity);
        places.push(p);
        return p;
    }

    function fn(uint8 vectorSize, uint8 action, uint8 role, Uint8Model.Position memory position) internal returns (Uint8Model.Transition memory) {
        require(uint8(transitions.length) == action, "Transition offset must match Actions enum");
        Uint8Model.Transition memory t = Uint8Model.Transition(action, position, role, new int8[](vectorSize), new int8[](vectorSize));
        transitions.push(t);
        return t;
    }
